...
import 'package:study_app/common/models/msgcontent.dart';
import 'package:study_app/common/models/user.dart';
import 'package:study_app/common/routes/app_routes_names.dart';
import 'package:study_app/common/utils/security.dart';
import 'package:study_app/common/widgets/popup_messages.dart';
import 'package:study_app/features/message/chat/notifiers/chat_notifier.dart';
...
class ChatLogic {
  ...
  void init() {
    final data = ModalRoute.of(ref.context)?.settings.arguments;

    if (data == null) {
      print("Error: No data passed via ModalRoute.");
    ...
    if (data is Map<String, dynamic>) {
      ...
      doc_id = data["doc_id"] ?? "";
      ref.read(chatProvider.notifier).onProfileChanged(ProfileChanged(
        data["to_token"] ?? "",
        data["to_name"] ?? "",
        data["to_avatar"] ?? "",
        data["to_online"] ?? "1",
  ...
  Future uploadFile() async {
    ...
    print("Fileeeeeeeeeeeeeee: ${result.data}");
    print("Tileeeeeeeeeeeeeee: ${result.toString()}");
  ...
  sendMessage() async{
    ...
    if(sendcontent.isEmpty){
      toastInfo("Vui lòng nhập tin nhắn");
      return;
    }
  ...
  photoImg(Msgcontent item){
    Navigator.of(ref.context).pushNamed(AppRoutesNames.PHOTO,arguments: {"url": item.content});
  }
